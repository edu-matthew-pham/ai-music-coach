...
The block is HTML with a script in it. Gradio renders it
into the page, and from there the browser's own audio
engine does the work. That is worth knowing when reading
this file: the Python here writes JavaScript, and the
usual guarantees stop at the edge - nothing below runs
under pytest, and a fault shows up in a browser console
...
(function () {{
  ...
  async function play(from) {{
    ...
    const whole = document.getElementById("mixer-loop").checked;
    ...
    const readout = document.getElementById("mixer-time");
  ...
  function markLoop() {{
    const note = document.getElementById("mixer-loop-note");
    for (const bar of bars) {{
  ...
  const holder = document.getElementById("mixer-faders");
  ...
  document.getElementById("mixer-play").addEventListener(
    "click", function () {{ play(); }}
  );

  document.getElementById("mixer-clear").addEventListener(
    "click", function () {{
      loopFrom = null;
      loopTo = null;
      markLoop();
  ...
  document.getElementById("mixer-stop").addEventListener("click", stop);
}})();
</script>
"""
